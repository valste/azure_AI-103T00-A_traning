    ...
    for file_name in os.listdir(messages_folder):
        if file_name.endswith(".wav"):
            ...
            if result.reason == speech_sdk.ResultReason.RecognizedSpeech:
                ...
                transcripted_file = f"transcribted_{file_name.replace('.wav', '.txt')}"
                with open(transcripted_file, "w", encoding="utf-8") as file:
                    file.write(result.text)
                print(f"Transcription saved to {transcripted_file}")
