"""The routing agent acts as an orchestrator that handles user messages and
determines which remote agent should process the request."""
...
async def _get_initialized_routing_agent_sync() -> RoutingAgent:
    ...
    # the loop is already running since we're in an async context, so we can just await the async main function
    try:
